/*
* Copyright (c) 2024 Stephen O Driscoll
*
* Distributed under the MIT License (See accompanying file LICENSE)
* Official repository: https://github.com/Stephen-ODriscoll/PlutoUtils
*/

#include <gtest/gtest.h>

#include <pluto/string.hpp>

#ifdef _WIN32
#define TEST_WITH_EXTRA_ELEMS 1 // Tests char8_t, char16_t and char32_t as elems, elem arrays and strings
#endif

#if PLUTO_UTILS_HAS_CXX_20
#define TEST_CHAR8_ELEMS_1(check, function, x) \
    do \
    { \
        check(function(u8##x)); \
    } \
    while (false)
#else
#define TEST_CHAR8_ELEMS_1(check, function, x) \
    do {} while (false)
#endif

#ifdef TEST_WITH_EXTRA_ELEMS
#define TEST_EXTRA_ELEMS_1(check, function, x) \
    do \
    { \
        TEST_CHAR8_ELEMS_1(check, function, x); \
        check(function(u##x)); \
        check(function(U##x)); \
    } \
    while (false)
#else
#define TEST_EXTRA_ELEMS_1(check, function, x) \
    do {} while (false)
#endif

#define TEST_ALL_ELEMS_1(check, function, x) \
    do \
    { \
        check(function(x)); \
        check(function(L##x)); \
        TEST_EXTRA_ELEMS_1(check, function, x); \
    } \
    while (false)

#if PLUTO_UTILS_HAS_CXX_20
#define TEST_CHAR8_ELEMS_2(check, function, x, y) \
    do \
    { \
        check(function(u8##x), u8##y); \
    } \
    while (false)
#else
#define TEST_CHAR8_ELEMS_2(check, function, x, y) \
    do {} while(false)
#endif

#ifdef TEST_WITH_EXTRA_ELEMS
#define TEST_EXTRA_ELEMS_2(check, function, x, y) \
    do \
    { \
        TEST_CHAR8_ELEMS_2(check, function, x, y); \
        check(function(u##x), u##y); \
        check(function(U##x), U##y); \
    } \
    while (false)
#else
#define TEST_EXTRA_ELEMS_2(check, function, x, y) \
    do {} while(false)
#endif

#define TEST_ALL_ELEMS_2(check, function, x, y) \
    do \
    { \
        check(function(x), y); \
        check(function(L##x), L##y); \
        TEST_EXTRA_ELEMS_2(check, function, x, y); \
    } \
    while (false)


#if PLUTO_UTILS_HAS_CXX_20
#define TEST_CHAR8_ELEM_ARRAYS_1(check, function, x) \
    do \
    { \
        check(function(u8##x, size)); \
    } \
    while (false)
#else
#define TEST_CHAR8_ELEM_ARRAYS_1(check, function, x) \
    do {} while(false)
#endif

#ifdef TEST_WITH_EXTRA_ELEMS
#define TEST_EXTRA_ELEM_ARRAYS_1(check, function, x) \
    do \
    { \
        TEST_CHAR8_ELEM_ARRAYS_1(check, function, x); \
        check(function(u##x, size)); \
        check(function(U##x, size)); \
    } \
    while(false)
#else
#define TEST_EXTRA_ELEM_ARRAYS_1(check, function, x) \
    do {} while(false)
#endif

#define TEST_ALL_ELEM_ARRAYS_1(check, function, x) \
    do \
    { \
        const auto size{ std::strlen(x) }; \
        \
        check(function(x, size)); \
        check(function(L##x, size)); \
        TEST_EXTRA_ELEM_ARRAYS_1(check, function, x); \
    } \
    while(false)

#if PLUTO_UTILS_HAS_CXX_20
#define TEST_CHAR8_ELEM_ARRAYS_2(check, function, x, y) \
    do \
    { \
        auto char8Array{ new char8_t[size + 1] }; \
        memcpy(char8Array, u8##x, ((size + 1) * sizeof(char8_t))); \
        function(char8Array, size); \
        check(std::u8string{ char8Array }, std::u8string{ u8##y }); \
        delete[] char8Array; \
    } \
    while (false)
#else
#define TEST_CHAR8_ELEM_ARRAYS_2(check, function, x, y) \
    do {} while(false)
#endif

#ifdef TEST_WITH_EXTRA_ELEMS
#define TEST_EXTRA_ELEM_ARRAYS_2(check, function, x, y) \
    do \
    { \
        TEST_CHAR8_ELEM_ARRAYS_2(check, function, x, y); \
        \
        auto char16Array{ new char16_t[size + 1] }; \
        memcpy(char16Array, u##x, ((size + 1) * sizeof(char16_t))); \
        function(char16Array, size); \
        check(std::u16string{ char16Array }, std::u16string{ u##y }); \
        delete[] char16Array; \
        \
        auto char32Array{ new char32_t[size + 1] }; \
        memcpy(char32Array, U##x, ((size + 1) * sizeof(char32_t))); \
        function(char32Array, size); \
        check(std::u32string{ char32Array }, std::u32string{ U##y }); \
        delete[] char32Array; \
    } \
    while (false)
#else
#define TEST_EXTRA_ELEM_ARRAYS_2(check, function, x, y) \
    do {} while(false)
#endif

#define TEST_ALL_ELEM_ARRAYS_2(check, function, x, y) \
    do \
    { \
        const auto size{ std::strlen(x) }; \
        \
        auto charArray{ new char[size + 1] }; \
        memcpy(charArray, x, ((size + 1) * sizeof(char))); \
        function(charArray, size); \
        check(std::string{ charArray }, std::string{ y }); \
        delete[] charArray; \
        \
        auto wcharArray{ new wchar_t[size + 1] }; \
        memcpy(wcharArray, L##x, ((size + 1) * sizeof(wchar_t))); \
        function(wcharArray, size); \
        check(std::wstring{ wcharArray }, std::wstring{ L##y }); \
        delete[] wcharArray; \
        \
        TEST_EXTRA_ELEM_ARRAYS_2(check, function, x, y); \
    } \
    while (false)

#if PLUTO_UTILS_HAS_CXX_20
#define TEST_CHAR8_ELEM_ARRAYS_3(check, function, x, y) \
    do \
    { \
        check(function(u8##x, xSize, u8##y, ySize)); \
    } \
    while (false)
#else
#define TEST_CHAR8_ELEM_ARRAYS_3(check, function, x, y) \
    do {} while(false)
#endif

#ifdef TEST_WITH_EXTRA_ELEMS
#define TEST_EXTRA_ELEM_ARRAYS_3(check, function, x, y) \
    do \
    { \
        TEST_CHAR8_ELEM_ARRAYS_3(check, function, x, y); \
        check(function(u##x, xSize, u##y, ySize)); \
        check(function(U##x, xSize, U##y, ySize)); \
    } \
    while(false)
#else
#define TEST_EXTRA_ELEM_ARRAYS_3(check, function, x, y) \
    do {} while(false)
#endif

#define TEST_ALL_ELEM_ARRAYS_3(check, function, x, y) \
    do \
    { \
        const auto xSize{ std::strlen(x) }; \
        const auto ySize{ std::strlen(y) }; \
        \
        check(function(x, xSize, y, ySize)); \
        check(function(L##x, xSize, L##y, ySize)); \
        TEST_EXTRA_ELEM_ARRAYS_3(check, function, x, y); \
    } \
    while(false)

#if PLUTO_UTILS_HAS_CXX_20
#define TEST_CHAR8_ELEM_ARRAYS_4(check, function, x, y, z) \
    do \
    { \
        const auto u8str{ u8##x }; \
        check(function(u8str, xSize, u8##y, ySize), u8str + z); \
    } \
    while (false)
#else
#define TEST_CHAR8_ELEM_ARRAYS_4(check, function, x, y, z) \
    do {} while(false)
#endif

#ifdef TEST_WITH_EXTRA_ELEMS
#define TEST_EXTRA_ELEM_ARRAYS_4(check, function, x, y, z) \
    do \
    { \
        TEST_CHAR8_ELEM_ARRAYS_4(check, function, x, y, z); \
        \
        const auto u16str{ u##x }; \
        check(function(u16str, xSize, u##y, ySize), u16str + z); \
        \
        const auto u32str{ U##x }; \
        check(function(u32str, xSize, U##y, ySize), u32str + z); \
    } \
    while(false)
#else
#define TEST_EXTRA_ELEM_ARRAYS_4(check, function, x, y, z) \
    do {} while(false)
#endif

#define TEST_ALL_ELEM_ARRAYS_4(check, function, x, y, z) \
    do \
    { \
        const auto xSize{ std::strlen(x) }; \
        const auto ySize{ std::strlen(y) }; \
        \
        const auto str{ x }; \
        check(function(str, xSize, y, ySize), str + z); \
        \
        const auto wstr{ L##x }; \
        check(function(wstr, xSize, L##y, ySize), wstr + z); \
        \
        TEST_EXTRA_ELEM_ARRAYS_4(check, function, x, y, z); \
    } \
    while(false)


#if PLUTO_UTILS_HAS_CXX_20
#define TEST_CHAR8_STRINGS_1(check, function, x) \
    do \
    { \
        check(function({ u8##x })); \
    } \
    while (false)
#else
#define TEST_CHAR8_STRINGS_1(check, function, x) \
    do {} while(false)
#endif

#ifdef TEST_WITH_EXTRA_ELEMS
#define TEST_EXTRA_STRINGS_1(check, function, x) \
    do \
    { \
        TEST_CHAR8_STRINGS_1(check, function, x); \
        check(function({ u##x })); \
        check(function({ U##x })); \
    } \
    while (false)
#else
#define TEST_EXTRA_STRINGS_1(check, function, x) \
    do {} while(false)
#endif

#define TEST_ALL_STRINGS_1(check, function, x) \
    do \
    { \
        check(function({ x })); \
        check(function({ L##x })); \
        TEST_EXTRA_STRINGS_1(check, function, x); \
    } \
    while (false)

#if PLUTO_UTILS_HAS_CXX_20
#define TEST_CHAR8_STRINGS_2(check, function, x, y) \
    do \
    { \
        std::u8string u8str{ u8##x }; \
        function(u8str); \
        check(u8str, u8##y); \
    } \
    while (false)
#else
#define TEST_CHAR8_STRINGS_2(check, function, x, y) \
    do {} while(false)
#endif

#ifdef TEST_WITH_EXTRA_ELEMS
#define TEST_EXTRA_STRINGS_2(check, function, x, y) \
    do \
    { \
        TEST_CHAR8_STRINGS_2(check, function, x, y); \
        \
        std::u16string u16str{ u##x }; \
        function(u16str); \
        check(u16str, u##y); \
        \
        std::u32string u32str{ U##x }; \
        function(u32str); \
        check(u32str, U##y); \
    } \
    while (false)
#else
#define TEST_EXTRA_STRINGS_2(check, function, x, y) \
    do {} while(false)
#endif

#define TEST_ALL_STRINGS_2(check, function, x, y) \
    do \
    { \
        std::string str{ x }; \
        function(str); \
        check(str, y); \
        \
        std::wstring wstr{ L##x }; \
        function(wstr); \
        check(wstr, L##y); \
        \
        TEST_EXTRA_STRINGS_2(check, function, x, y); \
    } \
    while (false)

#if PLUTO_UTILS_HAS_CXX_20
#define TEST_CHAR8_STRINGS_3(check, function, x, y) \
    do \
    { \
        check(function({ u8##x }, u8##y)); \
    } \
    while (false)
#else
#define TEST_CHAR8_STRINGS_3(check, function, x, y) \
    do {} while(false)
#endif

#ifdef TEST_WITH_EXTRA_ELEMS
#define TEST_EXTRA_STRINGS_3(check, function, x, y) \
    do \
    { \
        TEST_CHAR8_STRINGS_3(check, function, x, y); \
        check(function({ u##x }, u##y)); \
        check(function({ U##x }, U##y)); \
    } \
    while (false)
#else
#define TEST_EXTRA_STRINGS_3(check, function, x, y) \
    do {} while(false)
#endif

#define TEST_ALL_STRINGS_3(check, function, x, y) \
    do \
    { \
        check(function({ x }, y)); \
        check(function({ L##x }, L##y)); \
        TEST_EXTRA_STRINGS_3(check, function, x, y); \
    } \
    while (false)

#if PLUTO_UTILS_HAS_CXX_20
#define TEST_CHAR8_STRINGS_4(check, function, x, y, z) \
    do \
    { \
        std::u8string u8str{ u8##x }; \
        check(function(u8str, u8##y), u8str.begin() + z); \
    } \
    while (false)
#else
#define TEST_CHAR8_STRINGS_4(check, function, x, y, z) \
    do {} while(false)
#endif

#ifdef TEST_WITH_EXTRA_ELEMS
#define TEST_EXTRA_STRINGS_4(check, function, x, y, z) \
    do \
    { \
        TEST_CHAR8_STRINGS_4(check, function, x, y, z); \
        \
        std::u16string u16str{ u##x }; \
        check(function(u16str, u##y), u16str.begin() + z); \
        \
        std::u32string u32str{ U##x }; \
        check(function(u32str, U##y), u32str.begin() + z); \
    } \
    while (false)
#else
#define TEST_EXTRA_STRINGS_4(check, function, x, y, z) \
    do {} while(false)
#endif

#define TEST_ALL_STRINGS_4(check, function, x, y, z) \
    do \
    { \
        std::string str{ x }; \
        check(function(str, y), str.begin() + z); \
        \
        std::wstring wstr{ L##x }; \
        check(function(wstr, L##y), wstr.begin() + z); \
        \
        TEST_EXTRA_STRINGS_4(check, function, x, y, z); \
    } \
    while (false)

#define VECTOR_HELPER_0(pre, x)
#define VECTOR_HELPER_1(pre, x)         pre##x
#define VECTOR_HELPER_2(pre, x, ...)    pre##x, VECTOR_HELPER_1(pre, __VA_ARGS__)
#define VECTOR_HELPER_3(pre, x, ...)    pre##x, VECTOR_HELPER_2(pre, __VA_ARGS__)
#define VECTOR_HELPER_4(pre, x, ...)    pre##x, VECTOR_HELPER_3(pre, __VA_ARGS__)
#define VECTOR_HELPER_5(pre, x, ...)    pre##x, VECTOR_HELPER_4(pre, __VA_ARGS__)
#define VECTOR_HELPER_6(pre, x, ...)    pre##x, VECTOR_HELPER_5(pre, __VA_ARGS__)
#define VECTOR_HELPER_7(pre, x, ...)    pre##x, VECTOR_HELPER_6(pre, __VA_ARGS__)
#define VECTOR_HELPER_8(pre, x, ...)    pre##x, VECTOR_HELPER_7(pre, __VA_ARGS__)
#define VECTOR_HELPER_9(pre, x, ...)    pre##x, VECTOR_HELPER_8(pre, __VA_ARGS__)

#if PLUTO_UTILS_HAS_CXX_20
#define TEST_CHAR8_STRINGS_5(check, function, x, size, ...) \
    do \
    { \
        std::vector<std::u8string> u8splits{ VECTOR_HELPER_##size(u8, __VA_ARGS__) }; \
        check(function(u8##x), u8splits); \
    } \
    while (false)
#else
#define TEST_CHAR8_STRINGS_5(check, function, x, size, ...) \
    do {} while(false)
#endif

#ifdef TEST_WITH_EXTRA_ELEMS
#define TEST_EXTRA_STRINGS_5(check, function, x, size, ...) \
    do \
    { \
        TEST_CHAR8_STRINGS_5(check, function, x, size, __VA_ARGS__); \
        \
        std::vector<std::u16string> u16splits{ VECTOR_HELPER_##size(u, __VA_ARGS__) }; \
        check(function(u##x), u16splits); \
        \
        std::vector<std::u32string> u32splits{ VECTOR_HELPER_##size(U, __VA_ARGS__) }; \
        check(function(U##x), u32splits); \
    } \
    while (false)
#else
#define TEST_EXTRA_STRINGS_5(check, function, x, size, ...) \
    do {} while(false)
#endif

#define TEST_ALL_STRINGS_5(check, function, x, size, ...) \
    do \
    { \
        std::vector<std::string> splits{ VECTOR_HELPER_##size(, __VA_ARGS__) }; \
        check(function(x), splits); \
        \
        std::vector<std::wstring> wsplits{ VECTOR_HELPER_##size(L, __VA_ARGS__) }; \
        check(function(L##x), wsplits); \
        \
        TEST_EXTRA_STRINGS_5(check, function, x, size, __VA_ARGS__); \
    } \
    while (false)

#if PLUTO_UTILS_HAS_CXX_20
#define TEST_CHAR8_STRINGS_6(check, function, x, y, size, ...) \
    do \
    { \
        std::vector<std::u8string> u8splits{ VECTOR_HELPER_##size(u8, __VA_ARGS__) }; \
        check(function(u8##x, u8##y), u8splits); \
    } \
    while (false)
#else
#define TEST_CHAR8_STRINGS_6(check, function, x, y, size, ...) \
    do {} while(false)
#endif

#ifdef TEST_WITH_EXTRA_ELEMS
#define TEST_EXTRA_STRINGS_6(check, function, x, y, size, ...) \
    do \
    { \
        TEST_CHAR8_STRINGS_6(check, function, x, y, size, __VA_ARGS__); \
        \
        std::vector<std::u16string> u16splits{ VECTOR_HELPER_##size(u, __VA_ARGS__) }; \
        check(function(u##x, u##y), u16splits); \
        \
        std::vector<std::u32string> u32splits{ VECTOR_HELPER_##size(U, __VA_ARGS__) }; \
        check(function(U##x, U##y), u32splits); \
    } \
    while (false)
#else
#define TEST_EXTRA_STRINGS_6(check, function, x, y, size, ...) \
    do {} while(false)
#endif

#define TEST_ALL_STRINGS_6(check, function, x, y, size, ...) \
    do \
    { \
        std::vector<std::string> splits{ VECTOR_HELPER_##size(, __VA_ARGS__) }; \
        check(function(x, y), splits); \
        \
        std::vector<std::wstring> wsplits{ VECTOR_HELPER_##size(L, __VA_ARGS__) }; \
        check(function(L##x, L##y), wsplits); \
        \
        TEST_EXTRA_STRINGS_6(check, function, x, y, size, __VA_ARGS__); \
    } \
    while (false)

#if PLUTO_UTILS_HAS_CXX_20
#define TEST_CHAR8_STRINGS_7(check, function, x, y, size, ...) \
    do \
    { \
        std::vector<std::u8string> u8splits{ VECTOR_HELPER_##size(u8, __VA_ARGS__) }; \
        check(function(u8splits.begin(), u8splits.end(), u8##x), u8##y); \
    } \
    while (false)
#else
#define TEST_CHAR8_STRINGS_7(check, function, x, y, size, ...) \
    do {} while(false)
#endif

#ifdef TEST_WITH_EXTRA_ELEMS
#define TEST_EXTRA_STRINGS_7(check, function, x, y, size, ...) \
    do \
    { \
        TEST_CHAR8_STRINGS_7(check, function, x, y, size, __VA_ARGS__); \
        \
        std::vector<std::u16string> u16splits{ VECTOR_HELPER_##size(u, __VA_ARGS__) }; \
        check(function(u16splits.begin(), u16splits.end(), u##x), u##y); \
        \
        std::vector<std::u32string> u32splits{ VECTOR_HELPER_##size(U, __VA_ARGS__) }; \
        check(function(u32splits.begin(), u32splits.end(), U##x), U##y); \
    } \
    while (false)
#else
#define TEST_EXTRA_STRINGS_7(check, function, x, y, size, ...) \
    do {} while(false)
#endif

#define TEST_ALL_STRINGS_7(check, function, x, y, size, ...) \
    do \
    { \
        std::vector<std::string> splits{ VECTOR_HELPER_##size(, __VA_ARGS__) }; \
        check(function(splits.begin(), splits.end(), x), y); \
        \
        std::vector<std::wstring> wsplits{ VECTOR_HELPER_##size(L, __VA_ARGS__) }; \
        check(function(wsplits.begin(), wsplits.end(), L##x), L##y); \
        \
        TEST_EXTRA_STRINGS_7(check, function, x, y, size, __VA_ARGS__); \
    } \
    while (false)

#if PLUTO_UTILS_HAS_CXX_20
#define TEST_CHAR8_STRINGS_8(check, function, x, y, size, ...) \
    do \
    { \
        std::vector<std::u8string> u8splits{ VECTOR_HELPER_##size(u8, __VA_ARGS__) }; \
        check(function(u8splits, u8##x), u8##y); \
    } \
    while (false)
#else
#define TEST_CHAR8_STRINGS_8(check, function, x, y, size, ...) \
    do {} while(false)
#endif

#ifdef TEST_WITH_EXTRA_ELEMS
#define TEST_EXTRA_STRINGS_8(check, function, x, y, size, ...) \
    do \
    { \
        TEST_CHAR8_STRINGS_8(check, function, x, y, size, __VA_ARGS__); \
        \
        std::vector<std::u16string> u16splits{ VECTOR_HELPER_##size(u, __VA_ARGS__) }; \
        check(function(u16splits, u##x), u##y); \
        \
        std::vector<std::u32string> u32splits{ VECTOR_HELPER_##size(U, __VA_ARGS__) }; \
        check(function(u32splits, U##x), U##y); \
    } \
    while (false)
#else
#define TEST_EXTRA_STRINGS_8(check, function, x, y, size, ...) \
    do {} while(false)
#endif

#define TEST_ALL_STRINGS_8(check, function, x, y, size, ...) \
    do \
    { \
        std::vector<std::string> splits{ VECTOR_HELPER_##size(, __VA_ARGS__) }; \
        check(function(splits, x), y); \
        \
        std::vector<std::wstring> wsplits{ VECTOR_HELPER_##size(L, __VA_ARGS__) }; \
        check(function(wsplits, L##x), L##y); \
        \
        TEST_EXTRA_STRINGS_8(check, function, x, y, size, __VA_ARGS__); \
    } \
    while (false)

#if PLUTO_UTILS_HAS_CXX_20
#define TEST_CHAR8_STRINGS_9(check, function, x, y, z, a) \
    do \
    { \
        std::u8string u8str{ u8##x }; \
        function(u8str, u8##y, u8##z); \
        check(u8str, u8##a); \
    } \
    while (false)
#else
#define TEST_CHAR8_STRINGS_9(check, function, x, y, z, a) \
    do {} while(false)
#endif

#ifdef TEST_WITH_EXTRA_ELEMS
#define TEST_EXTRA_STRINGS_9(check, function, x, y, z, a) \
    do \
    { \
        TEST_CHAR8_STRINGS_9(check, function, x, y, z, a); \
        \
        std::u16string u16str{ u##x }; \
        function(u16str, u##y, u##z); \
        check(u16str, u##a); \
        \
        std::u32string u32str{ U##x }; \
        function(u32str, U##y, U##z); \
        check(u32str, U##a); \
    } \
    while (false)
#else
#define TEST_EXTRA_STRINGS_9(check, function, x, y, z, a) \
    do {} while(false)
#endif

#define TEST_ALL_STRINGS_9(check, function, x, y, z, a) \
    do \
    { \
        std::string str{ x }; \
        function(str, y, z); \
        check(str, a); \
        \
        std::wstring wstr{ L##x }; \
        function(wstr, L##y, L##z); \
        check(wstr, L##a); \
        \
        TEST_EXTRA_STRINGS_9(check, function, x, y, z, a); \
    } \
    while (false)

class string_tests : public testing::Test
{
protected:
    string_tests() {}
    ~string_tests() {}
};

TEST_F(string_tests, test_lower_use_elems)
{
    TEST_ALL_ELEMS_2(ASSERT_EQ, pluto::lower, ' ', ' ');
    TEST_ALL_ELEMS_2(ASSERT_EQ, pluto::lower, 'a', 'a');
    TEST_ALL_ELEMS_2(ASSERT_EQ, pluto::lower, 'A', 'a');
    TEST_ALL_ELEMS_2(ASSERT_NE, pluto::lower, 'a', 'A');
    TEST_ALL_ELEMS_2(ASSERT_NE, pluto::lower, 'A', 'A');
}

TEST_F(string_tests, test_lower_use_elem_arrays)
{
    TEST_ALL_ELEM_ARRAYS_2(ASSERT_EQ, pluto::lower, " ", " ");
    TEST_ALL_ELEM_ARRAYS_2(ASSERT_EQ, pluto::lower, "a", "a");
    TEST_ALL_ELEM_ARRAYS_2(ASSERT_EQ, pluto::lower, "A", "a");
    TEST_ALL_ELEM_ARRAYS_2(ASSERT_NE, pluto::lower, "a", "A");
    TEST_ALL_ELEM_ARRAYS_2(ASSERT_NE, pluto::lower, "A", "A");

    TEST_ALL_ELEM_ARRAYS_2(ASSERT_EQ, pluto::lower, "", "");
    TEST_ALL_ELEM_ARRAYS_2(ASSERT_EQ, pluto::lower, "abcdef", "abcdef");
    TEST_ALL_ELEM_ARRAYS_2(ASSERT_EQ, pluto::lower, "ABCDEF", "abcdef");
    TEST_ALL_ELEM_ARRAYS_2(ASSERT_NE, pluto::lower, "abcdef", "ABCDEF");
    TEST_ALL_ELEM_ARRAYS_2(ASSERT_NE, pluto::lower, "ABCDEF", "ABCDEF");

    TEST_ALL_ELEM_ARRAYS_2(ASSERT_EQ, pluto::lower, "aBcDeF", "abcdef");
    TEST_ALL_ELEM_ARRAYS_2(ASSERT_NE, pluto::lower, "abcdef", "aBcDeF");
}

TEST_F(string_tests, test_lower_use_strings)
{
    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::lower, " ", " ");
    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::lower, "a", "a");
    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::lower, "A", "a");
    TEST_ALL_STRINGS_2(ASSERT_NE, pluto::lower, "a", "A");
    TEST_ALL_STRINGS_2(ASSERT_NE, pluto::lower, "A", "A");

    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::lower, "", "");
    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::lower, "abcdef", "abcdef");
    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::lower, "ABCDEF", "abcdef");
    TEST_ALL_STRINGS_2(ASSERT_NE, pluto::lower, "abcdef", "ABCDEF");
    TEST_ALL_STRINGS_2(ASSERT_NE, pluto::lower, "ABCDEF", "ABCDEF");

    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::lower, "aBcDeF", "abcdef");
    TEST_ALL_STRINGS_2(ASSERT_NE, pluto::lower, "abcdef", "aBcDeF");
}

TEST_F(string_tests, test_upper_use_elems)
{
    TEST_ALL_ELEMS_2(ASSERT_EQ, pluto::upper, ' ', ' ');
    TEST_ALL_ELEMS_2(ASSERT_EQ, pluto::upper, 'A', 'A');
    TEST_ALL_ELEMS_2(ASSERT_EQ, pluto::upper, 'a', 'A');
    TEST_ALL_ELEMS_2(ASSERT_NE, pluto::upper, 'A', 'a');
    TEST_ALL_ELEMS_2(ASSERT_NE, pluto::upper, 'a', 'a');
}

TEST_F(string_tests, test_upper_use_elem_arrays)
{
    TEST_ALL_ELEM_ARRAYS_2(ASSERT_EQ, pluto::upper, " ", " ");
    TEST_ALL_ELEM_ARRAYS_2(ASSERT_EQ, pluto::upper, "A", "A");
    TEST_ALL_ELEM_ARRAYS_2(ASSERT_EQ, pluto::upper, "a", "A");
    TEST_ALL_ELEM_ARRAYS_2(ASSERT_NE, pluto::upper, "A", "a");
    TEST_ALL_ELEM_ARRAYS_2(ASSERT_NE, pluto::upper, "a", "a");

    TEST_ALL_ELEM_ARRAYS_2(ASSERT_EQ, pluto::upper, "", "");
    TEST_ALL_ELEM_ARRAYS_2(ASSERT_EQ, pluto::upper, "ABCDEF", "ABCDEF");
    TEST_ALL_ELEM_ARRAYS_2(ASSERT_EQ, pluto::upper, "abcdef", "ABCDEF");
    TEST_ALL_ELEM_ARRAYS_2(ASSERT_NE, pluto::upper, "ABCDEF", "abcdef");
    TEST_ALL_ELEM_ARRAYS_2(ASSERT_NE, pluto::upper, "abcdef", "abcdef");

    TEST_ALL_ELEM_ARRAYS_2(ASSERT_EQ, pluto::upper, "aBcDeF", "ABCDEF");
    TEST_ALL_ELEM_ARRAYS_2(ASSERT_NE, pluto::upper, "ABCDEF", "aBcDeF");
}

TEST_F(string_tests, test_upper_use_strings)
{
    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::upper, " ", " ");
    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::upper, "A", "A");
    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::upper, "a", "A");
    TEST_ALL_STRINGS_2(ASSERT_NE, pluto::upper, "A", "a");
    TEST_ALL_STRINGS_2(ASSERT_NE, pluto::upper, "a", "a");

    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::upper, "", "");
    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::upper, "ABCDEF", "ABCDEF");
    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::upper, "abcdef", "ABCDEF");
    TEST_ALL_STRINGS_2(ASSERT_NE, pluto::upper, "ABCDEF", "abcdef");
    TEST_ALL_STRINGS_2(ASSERT_NE, pluto::upper, "abcdef", "abcdef");

    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::upper, "aBcDeF", "ABCDEF");
    TEST_ALL_STRINGS_2(ASSERT_NE, pluto::upper, "ABCDEF", "aBcDeF");
}

TEST_F(string_tests, test_is_lower_use_elems)
{
    TEST_ALL_ELEMS_1(ASSERT_TRUE, pluto::is_lower, ' ');
    TEST_ALL_ELEMS_1(ASSERT_TRUE, pluto::is_lower, 'a');
    TEST_ALL_ELEMS_1(ASSERT_FALSE, pluto::is_lower, 'A');
}

TEST_F(string_tests, test_is_lower_use_elem_arrays)
{
    TEST_ALL_ELEM_ARRAYS_1(ASSERT_TRUE, pluto::is_lower, " ");
    TEST_ALL_ELEM_ARRAYS_1(ASSERT_TRUE, pluto::is_lower, "a");
    TEST_ALL_ELEM_ARRAYS_1(ASSERT_FALSE, pluto::is_lower, "A");

    TEST_ALL_ELEM_ARRAYS_1(ASSERT_TRUE, pluto::is_lower, "");
    TEST_ALL_ELEM_ARRAYS_1(ASSERT_TRUE, pluto::is_lower, "abcdef");
    TEST_ALL_ELEM_ARRAYS_1(ASSERT_FALSE, pluto::is_lower, "ABCDEF");

    TEST_ALL_ELEM_ARRAYS_1(ASSERT_FALSE, pluto::is_lower, "aBcDeF");
}

TEST_F(string_tests, test_is_lower_use_strings)
{
    TEST_ALL_STRINGS_1(ASSERT_TRUE, pluto::is_lower, " ");
    TEST_ALL_STRINGS_1(ASSERT_TRUE, pluto::is_lower, "a");
    TEST_ALL_STRINGS_1(ASSERT_FALSE, pluto::is_lower, "A");

    TEST_ALL_STRINGS_1(ASSERT_TRUE, pluto::is_lower, "");
    TEST_ALL_STRINGS_1(ASSERT_TRUE, pluto::is_lower, "abcdef");
    TEST_ALL_STRINGS_1(ASSERT_FALSE, pluto::is_lower, "ABCDEF");

    TEST_ALL_STRINGS_1(ASSERT_FALSE, pluto::is_lower, "aBcDeF");
}

TEST_F(string_tests, test_is_upper_use_elems)
{
    TEST_ALL_ELEMS_1(ASSERT_TRUE, pluto::is_upper, ' ');
    TEST_ALL_ELEMS_1(ASSERT_TRUE, pluto::is_upper, 'A');
    TEST_ALL_ELEMS_1(ASSERT_FALSE, pluto::is_upper, 'a');
}

TEST_F(string_tests, test_is_upper_use_elem_arrays)
{
    TEST_ALL_ELEM_ARRAYS_1(ASSERT_TRUE, pluto::is_upper, " ");
    TEST_ALL_ELEM_ARRAYS_1(ASSERT_TRUE, pluto::is_upper, "A");
    TEST_ALL_ELEM_ARRAYS_1(ASSERT_FALSE, pluto::is_upper, "a");

    TEST_ALL_ELEM_ARRAYS_1(ASSERT_TRUE, pluto::is_upper, "");
    TEST_ALL_ELEM_ARRAYS_1(ASSERT_TRUE, pluto::is_upper, "ABCDEF");
    TEST_ALL_ELEM_ARRAYS_1(ASSERT_FALSE, pluto::is_upper, "abcdef");

    TEST_ALL_ELEM_ARRAYS_1(ASSERT_FALSE, pluto::is_upper, "aBcDeF");
}

TEST_F(string_tests, test_is_upper_use_strings)
{
    TEST_ALL_STRINGS_1(ASSERT_TRUE, pluto::is_upper, " ");
    TEST_ALL_STRINGS_1(ASSERT_TRUE, pluto::is_upper, "A");
    TEST_ALL_STRINGS_1(ASSERT_FALSE, pluto::is_upper, "a");

    TEST_ALL_STRINGS_1(ASSERT_TRUE, pluto::is_upper, "");
    TEST_ALL_STRINGS_1(ASSERT_TRUE, pluto::is_upper, "ABCDEF");
    TEST_ALL_STRINGS_1(ASSERT_FALSE, pluto::is_upper, "abcdef");

    TEST_ALL_STRINGS_1(ASSERT_FALSE, pluto::is_upper, "aBcDeF");
}

TEST_F(string_tests, test_str_use_char_array)
{
    const auto pChar{ "abcdef" };
    const auto size { std::strlen(pChar) };

    ASSERT_EQ(pluto::str(pChar, size), std::string(pChar));
}

TEST_F(string_tests, test_str_use_string)
{
    const std::string str{ "abcdef" };

    ASSERT_EQ(pluto::str(str), str);
}

TEST_F(string_tests, test_str_use_wchar_array)
{
    const auto pWChar   { L"abcdef" };
    const auto pChar    { "abcdef" };
    const auto size     { std::wcslen(pWChar) };

    ASSERT_EQ(pluto::str(pWChar, size), std::string(pChar));
}

TEST_F(string_tests, test_str_use_wstring)
{
    const std::wstring wstr { L"abcdef" };
    const std::string str   { "abcdef" };

    ASSERT_EQ(pluto::str(wstr), str);
}

TEST_F(string_tests, test_str_use_values)
{
    const std::string str{ "1" };

    ASSERT_EQ(pluto::str(1),    str);
    ASSERT_EQ(pluto::str(1l),   str);
    ASSERT_EQ(pluto::str(1ll),  str);
    ASSERT_EQ(pluto::str(1u),   str);
    ASSERT_EQ(pluto::str(1ul),  str);
    ASSERT_EQ(pluto::str(1ull), str);
    ASSERT_EQ(pluto::str(1.0),  str);
    ASSERT_EQ(pluto::str(1.0f), str);
}

TEST_F(string_tests, test_wstr_use_wchar_array)
{
    const auto pWChar   { L"abcdef" };
    const auto size     { std::wcslen(pWChar) };

    ASSERT_EQ(pluto::wstr(pWChar, size), std::wstring(pWChar));
}

TEST_F(string_tests, test_wstr_use_wstring)
{
    const std::wstring wstr{ L"abcdef" };

    ASSERT_EQ(pluto::wstr(wstr), wstr);
}

TEST_F(string_tests, test_wstr_use_char_array)
{
    const auto pWChar   { L"abcdef" };
    const auto pChar    { "abcdef" };
    const auto size     { std::strlen(pChar) };

    ASSERT_EQ(pluto::wstr(pChar, size), std::wstring(pWChar));
}

TEST_F(string_tests, test_wstr_use_string)
{
    const std::wstring wstr { L"abcdef" };
    const std::string str   { "abcdef" };

    ASSERT_EQ(pluto::wstr(str), wstr);
}

TEST_F(string_tests, test_wstr_use_values)
{
    const std::wstring wstr{ L"1" };

    ASSERT_EQ(pluto::wstr(1),       wstr);
    ASSERT_EQ(pluto::wstr(1l),      wstr);
    ASSERT_EQ(pluto::wstr(1ll),     wstr);
    ASSERT_EQ(pluto::wstr(1u),      wstr);
    ASSERT_EQ(pluto::wstr(1ul),     wstr);
    ASSERT_EQ(pluto::wstr(1ull),    wstr);
    ASSERT_EQ(pluto::wstr(1.0),     wstr);
    ASSERT_EQ(pluto::wstr(1.0f),    wstr);
}

TEST_F(string_tests, test_str_to_values_use_string)
{
    const std::string str{ "1" };

    ASSERT_EQ(pluto::str_to<int>(str),                  1);
    ASSERT_EQ(pluto::str_to<long>(str),                 1l);
    ASSERT_EQ(pluto::str_to<long long>(str),            1ll);
    ASSERT_EQ(pluto::str_to<unsigned int>(str),         1u);
    ASSERT_EQ(pluto::str_to<unsigned long>(str),        1ul);
    ASSERT_EQ(pluto::str_to<unsigned long long>(str),   1ull);
    ASSERT_EQ(pluto::str_to<double>(str),               1.0);
    ASSERT_EQ(pluto::str_to<float>(str),                1.0f);
}

TEST_F(string_tests, test_str_to_values_use_char_array)
{
    const auto pChar{ "1" };
    const auto size { std::strlen(pChar) };

    ASSERT_EQ(pluto::str_to<int>(pChar, size),                  1);
    ASSERT_EQ(pluto::str_to<long>(pChar, size),                 1l);
    ASSERT_EQ(pluto::str_to<long long>(pChar, size),            1ll);
    ASSERT_EQ(pluto::str_to<unsigned int>(pChar, size),         1u);
    ASSERT_EQ(pluto::str_to<unsigned long>(pChar, size),        1ul);
    ASSERT_EQ(pluto::str_to<unsigned long long>(pChar, size),   1ull);
    ASSERT_EQ(pluto::str_to<double>(pChar, size),               1.0);
    ASSERT_EQ(pluto::str_to<float>(pChar, size),                1.0f);
}

TEST_F(string_tests, test_wstr_to_values_use_wstring)
{
    const std::wstring wstr{ L"1" };

    ASSERT_EQ(pluto::wstr_to<int>(wstr),                1);
    ASSERT_EQ(pluto::wstr_to<long>(wstr),               1l);
    ASSERT_EQ(pluto::wstr_to<long long>(wstr),          1ll);
    ASSERT_EQ(pluto::wstr_to<unsigned int>(wstr),       1u);
    ASSERT_EQ(pluto::wstr_to<unsigned long>(wstr),      1ul);
    ASSERT_EQ(pluto::wstr_to<unsigned long long>(wstr), 1ull);
    ASSERT_EQ(pluto::wstr_to<double>(wstr),             1.0);
    ASSERT_EQ(pluto::wstr_to<float>(wstr),              1.0f);
}

TEST_F(string_tests, test_wstr_to_values_use_wchar_array)
{
    const auto pWChar   { L"1" };
    const auto size     { std::wcslen(pWChar) };

    ASSERT_EQ(pluto::wstr_to<int>(pWChar, size),                1);
    ASSERT_EQ(pluto::wstr_to<long>(pWChar, size),               1l);
    ASSERT_EQ(pluto::wstr_to<long long>(pWChar, size),          1ll);
    ASSERT_EQ(pluto::wstr_to<unsigned int>(pWChar, size),       1u);
    ASSERT_EQ(pluto::wstr_to<unsigned long>(pWChar, size),      1ul);
    ASSERT_EQ(pluto::wstr_to<unsigned long long>(pWChar, size), 1ull);
    ASSERT_EQ(pluto::wstr_to<double>(pWChar, size),             1.0);
    ASSERT_EQ(pluto::wstr_to<float>(pWChar, size),              1.0f);
}

TEST_F(string_tests, test_equals_use_strings)
{
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::equals, " ", " ");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::equals, "A", "A");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::equals, "a", "A");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::equals, "A", "a");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::equals, "a", "a");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::equals, "A", "B");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::equals, "a", "B");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::equals, "A", "b");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::equals, "a", "b");

    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::equals, "", "");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::equals, "ABCDEF", "ABCDEF");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::equals, "abcdef", "ABCDEF");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::equals, "ABCDEF", "abcdef");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::equals, "abcdef", "abcdef");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::equals, " ", "");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::equals, "", " ");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::equals, "ABCDEF", "ABCDEG");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::equals, "abcdef", "ABCDEG");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::equals, "ABCDEF", "abcdeg");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::equals, "abcdef", "abcdeg");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::equals, "ABCDEF", "ABCDE");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::equals, "abcdef", "ABCDE");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::equals, "ABCDEF", "abcde");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::equals, "abcdef", "abcde");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::equals, "ABCDEF", "BCDEF");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::equals, "abcdef", "BCDEF");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::equals, "ABCDEF", "bcdef");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::equals, "abcdef", "bcdef");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::equals, "ABCDEF", "BCDE");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::equals, "abcdef", "BCDE");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::equals, "ABCDEF", "bcde");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::equals, "abcdef", "bcde");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::equals, "aBcDeF", "ABCDEF");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::equals, "ABCDEF", "aBcDeF");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::equals, "aBcDeF", "ABCDEG");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::equals, "ABCDEF", "aBcDeg");
}

TEST_F(string_tests, test_iequals_use_elem_arrays)
{
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::iequals, " ", " ");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::iequals, "A", "A");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::iequals, "a", "A");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::iequals, "A", "a");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::iequals, "a", "a");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iequals, "A", "B");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iequals, "a", "B");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iequals, "A", "b");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iequals, "a", "b");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::iequals, "", "");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::iequals, "ABCDEF", "ABCDEF");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::iequals, "abcdef", "ABCDEF");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::iequals, "ABCDEF", "abcdef");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::iequals, "abcdef", "abcdef");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iequals, " ", "");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iequals, "", " ");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iequals, "ABCDEF", "ABCDEG");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iequals, "abcdef", "ABCDEG");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iequals, "ABCDEF", "abcdeg");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iequals, "abcdef", "abcdeg");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iequals, "ABCDEF", "ABCDE");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iequals, "abcdef", "ABCDE");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iequals, "ABCDEF", "abcde");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iequals, "abcdef", "abcde");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iequals, "ABCDEF", "BCDEF");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iequals, "abcdef", "BCDEF");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iequals, "ABCDEF", "bcdef");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iequals, "abcdef", "bcdef");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iequals, "ABCDEF", "BCDE");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iequals, "abcdef", "BCDE");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iequals, "ABCDEF", "bcde");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iequals, "abcdef", "bcde");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::iequals, "aBcDeF", "ABCDEF");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::iequals, "ABCDEF", "aBcDeF");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iequals, "aBcDeF", "ABCDEG");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iequals, "ABCDEF", "aBcDeg");
}

TEST_F(string_tests, test_iequals_use_strings)
{
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::iequals, " ", " ");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::iequals, "A", "A");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::iequals, "a", "A");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::iequals, "A", "a");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::iequals, "a", "a");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iequals, "A", "B");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iequals, "a", "B");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iequals, "A", "b");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iequals, "a", "b");

    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::iequals, "", "");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::iequals, "ABCDEF", "ABCDEF");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::iequals, "abcdef", "ABCDEF");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::iequals, "ABCDEF", "abcdef");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::iequals, "abcdef", "abcdef");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iequals, " ", "");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iequals, "", " ");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iequals, "ABCDEF", "ABCDEG");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iequals, "abcdef", "ABCDEG");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iequals, "ABCDEF", "abcdeg");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iequals, "abcdef", "abcdeg");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iequals, "ABCDEF", "ABCDE");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iequals, "abcdef", "ABCDE");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iequals, "ABCDEF", "abcde");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iequals, "abcdef", "abcde");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iequals, "ABCDEF", "BCDEF");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iequals, "abcdef", "BCDEF");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iequals, "ABCDEF", "bcdef");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iequals, "abcdef", "bcdef");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iequals, "ABCDEF", "BCDE");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iequals, "abcdef", "BCDE");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iequals, "ABCDEF", "bcde");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iequals, "abcdef", "bcde");

    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::iequals, "aBcDeF", "ABCDEF");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::iequals, "ABCDEF", "aBcDeF");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iequals, "aBcDeF", "ABCDEG");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iequals, "ABCDEF", "aBcDeg");
}

TEST_F(string_tests, test_starts_with_use_strings)
{
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::starts_with, " ", " ");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::starts_with, "A", "A");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::starts_with, "a", "A");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::starts_with, "A", "a");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::starts_with, "a", "a");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::starts_with, "A", "B");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::starts_with, "a", "B");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::starts_with, "A", "b");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::starts_with, "a", "b");

    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::starts_with, "", "");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::starts_with, " ", "");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::starts_with, "ABCDEF", "ABCDEF");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::starts_with, "abcdef", "ABCDEF");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::starts_with, "ABCDEF", "abcdef");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::starts_with, "abcdef", "abcdef");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::starts_with, "", " ");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::starts_with, "ABCDEF", "ABCDEG");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::starts_with, "abcdef", "ABCDEG");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::starts_with, "ABCDEF", "abcdeg");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::starts_with, "abcdef", "abcdeg");

    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::starts_with, "ABCDEF", "ABCDE");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::starts_with, "abcdef", "ABCDE");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::starts_with, "ABCDEF", "abcde");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::starts_with, "abcdef", "abcde");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::starts_with, "ABCDEF", "BCDEF");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::starts_with, "abcdef", "BCDEF");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::starts_with, "ABCDEF", "bcdef");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::starts_with, "abcdef", "bcdef");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::starts_with, "ABCDEF", "BCDE");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::starts_with, "abcdef", "BCDE");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::starts_with, "ABCDEF", "bcde");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::starts_with, "abcdef", "bcde");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::starts_with, "aBcDeF", "ABCDEF");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::starts_with, "ABCDEF", "aBcDeF");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::starts_with, "aBcDeF", "ABCDEG");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::starts_with, "ABCDEF", "aBcDeg");
}

TEST_F(string_tests, test_istarts_with_use_elem_arrays)
{
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::istarts_with, " ", " ");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::istarts_with, "A", "A");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::istarts_with, "a", "A");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::istarts_with, "A", "a");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::istarts_with, "a", "a");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::istarts_with, "A", "B");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::istarts_with, "a", "B");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::istarts_with, "A", "b");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::istarts_with, "a", "b");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::istarts_with, "", "");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::istarts_with, " ", "");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::istarts_with, "ABCDEF", "ABCDEF");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::istarts_with, "abcdef", "ABCDEF");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::istarts_with, "ABCDEF", "abcdef");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::istarts_with, "abcdef", "abcdef");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::istarts_with, "", " ");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::istarts_with, "ABCDEF", "ABCDEG");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::istarts_with, "abcdef", "ABCDEG");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::istarts_with, "ABCDEF", "abcdeg");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::istarts_with, "abcdef", "abcdeg");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::istarts_with, "ABCDEF", "ABCDE");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::istarts_with, "abcdef", "ABCDE");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::istarts_with, "ABCDEF", "abcde");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::istarts_with, "abcdef", "abcde");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::istarts_with, "ABCDEF", "BCDEF");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::istarts_with, "abcdef", "BCDEF");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::istarts_with, "ABCDEF", "bcdef");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::istarts_with, "abcdef", "bcdef");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::istarts_with, "ABCDEF", "BCDE");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::istarts_with, "abcdef", "BCDE");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::istarts_with, "ABCDEF", "bcde");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::istarts_with, "abcdef", "bcde");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::istarts_with, "aBcDeF", "ABCDEF");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::istarts_with, "ABCDEF", "aBcDeF");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::istarts_with, "aBcDeF", "ABCDEG");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::istarts_with, "ABCDEF", "aBcDeg");
}

TEST_F(string_tests, test_istarts_with_use_strings)
{
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::istarts_with, " ", " ");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::istarts_with, "A", "A");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::istarts_with, "a", "A");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::istarts_with, "A", "a");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::istarts_with, "a", "a");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::istarts_with, "A", "B");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::istarts_with, "a", "B");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::istarts_with, "A", "b");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::istarts_with, "a", "b");

    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::istarts_with, "", "");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::istarts_with, " ", "");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::istarts_with, "ABCDEF", "ABCDEF");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::istarts_with, "abcdef", "ABCDEF");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::istarts_with, "ABCDEF", "abcdef");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::istarts_with, "abcdef", "abcdef");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::istarts_with, "", " ");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::istarts_with, "ABCDEF", "ABCDEG");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::istarts_with, "abcdef", "ABCDEG");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::istarts_with, "ABCDEF", "abcdeg");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::istarts_with, "abcdef", "abcdeg");

    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::istarts_with, "ABCDEF", "ABCDE");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::istarts_with, "abcdef", "ABCDE");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::istarts_with, "ABCDEF", "abcde");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::istarts_with, "abcdef", "abcde");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::istarts_with, "ABCDEF", "BCDEF");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::istarts_with, "abcdef", "BCDEF");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::istarts_with, "ABCDEF", "bcdef");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::istarts_with, "abcdef", "bcdef");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::istarts_with, "ABCDEF", "BCDE");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::istarts_with, "abcdef", "BCDE");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::istarts_with, "ABCDEF", "bcde");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::istarts_with, "abcdef", "bcde");

    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::istarts_with, "aBcDeF", "ABCDEF");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::istarts_with, "ABCDEF", "aBcDeF");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::istarts_with, "aBcDeF", "ABCDEG");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::istarts_with, "ABCDEF", "aBcDeg");
}

TEST_F(string_tests, test_ends_with_use_strings)
{
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::ends_with, " ", " ");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::ends_with, "A", "A");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::ends_with, "a", "A");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::ends_with, "A", "a");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::ends_with, "a", "a");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::ends_with, "A", "B");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::ends_with, "a", "B");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::ends_with, "A", "b");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::ends_with, "a", "b");

    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::ends_with, "", "");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::ends_with, " ", "");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::ends_with, "ABCDEF", "ABCDEF");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::ends_with, "abcdef", "ABCDEF");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::ends_with, "ABCDEF", "abcdef");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::ends_with, "abcdef", "abcdef");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::ends_with, "", " ");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::ends_with, "ABCDEF", "ABCDEG");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::ends_with, "abcdef", "ABCDEG");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::ends_with, "ABCDEF", "abcdeg");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::ends_with, "abcdef", "abcdeg");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::ends_with, "ABCDEF", "ABCDE");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::ends_with, "abcdef", "ABCDE");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::ends_with, "ABCDEF", "abcde");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::ends_with, "abcdef", "abcde");

    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::ends_with, "ABCDEF", "BCDEF");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::ends_with, "abcdef", "BCDEF");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::ends_with, "ABCDEF", "bcdef");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::ends_with, "abcdef", "bcdef");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::ends_with, "ABCDEF", "BCDE");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::ends_with, "abcdef", "BCDE");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::ends_with, "ABCDEF", "bcde");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::ends_with, "abcdef", "bcde");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::ends_with, "aBcDeF", "ABCDEF");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::ends_with, "ABCDEF", "aBcDeF");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::ends_with, "aBcDeF", "ABCDEG");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::ends_with, "ABCDEF", "aBcDeg");
}

TEST_F(string_tests, test_iends_with_use_elem_arrays)
{
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::iends_with, " ", " ");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::iends_with, "A", "A");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::iends_with, "a", "A");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::iends_with, "A", "a");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::iends_with, "a", "a");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iends_with, "A", "B");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iends_with, "a", "B");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iends_with, "A", "b");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iends_with, "a", "b");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::iends_with, "", "");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::iends_with, " ", "");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::iends_with, "ABCDEF", "ABCDEF");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::iends_with, "abcdef", "ABCDEF");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::iends_with, "ABCDEF", "abcdef");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::iends_with, "abcdef", "abcdef");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iends_with, "", " ");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iends_with, "ABCDEF", "ABCDEG");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iends_with, "abcdef", "ABCDEG");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iends_with, "ABCDEF", "abcdeg");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iends_with, "abcdef", "abcdeg");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iends_with, "ABCDEF", "ABCDE");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iends_with, "abcdef", "ABCDE");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iends_with, "ABCDEF", "abcde");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iends_with, "abcdef", "abcde");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::iends_with, "ABCDEF", "BCDEF");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::iends_with, "abcdef", "BCDEF");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::iends_with, "ABCDEF", "bcdef");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::iends_with, "abcdef", "bcdef");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iends_with, "ABCDEF", "BCDE");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iends_with, "abcdef", "BCDE");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iends_with, "ABCDEF", "bcde");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iends_with, "abcdef", "bcde");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::iends_with, "aBcDeF", "ABCDEF");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::iends_with, "ABCDEF", "aBcDeF");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iends_with, "aBcDeF", "ABCDEG");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::iends_with, "ABCDEF", "aBcDeg");
}

TEST_F(string_tests, test_iends_with_use_strings)
{
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::iends_with, " ", " ");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::iends_with, "A", "A");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::iends_with, "a", "A");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::iends_with, "A", "a");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::iends_with, "a", "a");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iends_with, "A", "B");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iends_with, "a", "B");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iends_with, "A", "b");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iends_with, "a", "b");

    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::iends_with, "", "");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::iends_with, " ", "");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::iends_with, "ABCDEF", "ABCDEF");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::iends_with, "abcdef", "ABCDEF");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::iends_with, "ABCDEF", "abcdef");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::iends_with, "abcdef", "abcdef");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iends_with, "", " ");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iends_with, "ABCDEF", "ABCDEG");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iends_with, "abcdef", "ABCDEG");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iends_with, "ABCDEF", "abcdeg");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iends_with, "abcdef", "abcdeg");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iends_with, "ABCDEF", "ABCDE");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iends_with, "abcdef", "ABCDE");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iends_with, "ABCDEF", "abcde");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iends_with, "abcdef", "abcde");

    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::iends_with, "ABCDEF", "BCDEF");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::iends_with, "abcdef", "BCDEF");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::iends_with, "ABCDEF", "bcdef");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::iends_with, "abcdef", "bcdef");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iends_with, "ABCDEF", "BCDE");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iends_with, "abcdef", "BCDE");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iends_with, "ABCDEF", "bcde");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iends_with, "abcdef", "bcde");

    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::iends_with, "aBcDeF", "ABCDEF");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::iends_with, "ABCDEF", "aBcDeF");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iends_with, "aBcDeF", "ABCDEG");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::iends_with, "ABCDEF", "aBcDeg");
}

TEST_F(string_tests, test_find_use_strings)
{
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, " ", " ", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "A", "A", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "a", "A", 1); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "A", "a", 1); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "a", "a", 0);

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "A", "B", 1); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "a", "B", 1); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "A", "b", 1); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "a", "b", 1); // end

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "", "", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, " ", "", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "ABCDEF", "ABCDEF", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "abcdef", "ABCDEF", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "ABCDEF", "abcdef", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "abcdef", "abcdef", 0);

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "", " ", 0); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "ABCDEF", "ABCDEFG", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "abcdef", "ABCDEFG", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "ABCDEF", "abcdefg", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "abcdef", "abcdefg", 6); // end

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "ABCDEF", "ABCDE", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "abcdef", "ABCDE", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "ABCDEF", "abcde", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "abcdef", "abcde", 0);

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "ABCDEF", "BCDEF", 1);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "abcdef", "BCDEF", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "ABCDEF", "bcdef", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "abcdef", "bcdef", 1);

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "ABCDEF", "BCDE", 1);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "abcdef", "BCDE", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "ABCDEF", "bcde", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "abcdef", "bcde", 1);

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "aBcDeF", "ABCDEF", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "ABCDEF", "aBcDeF", 6); // end

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "aBcDeF", "ABCDEG", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::find, "ABCDEF", "aBcDeg", 6); // end
}

#if PLUTO_UTILS_HAS_CXX_17
TEST_F(string_tests, test_rfind_use_strings)
{
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, " ", " ", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "A", "A", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "a", "A", 1); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "A", "a", 1); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "a", "a", 0);

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "A", "B", 1); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "a", "B", 1); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "A", "b", 1); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "a", "b", 1); // end

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "", "", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, " ", "", 1);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "ABCDEF", "ABCDEF", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "abcdef", "ABCDEF", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "ABCDEF", "abcdef", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "abcdef", "abcdef", 0);

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "", " ", 0); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "ABCDEF", "ABCDEFG", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "abcdef", "ABCDEFG", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "ABCDEF", "abcdefg", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "abcdef", "abcdefg", 6); // end

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "ABCDEF", "ABCDE", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "abcdef", "ABCDE", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "ABCDEF", "abcde", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "abcdef", "abcde", 0);

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "ABCDEF", "BCDEF", 1);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "abcdef", "BCDEF", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "ABCDEF", "bcdef", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "abcdef", "bcdef", 1);

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "ABCDEF", "BCDE", 1);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "abcdef", "BCDE", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "ABCDEF", "bcde", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "abcdef", "bcde", 1);

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "aBcDeF", "ABCDEF", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "ABCDEF", "aBcDeF", 6); // end

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "aBcDeF", "ABCDEG", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::rfind, "ABCDEF", "aBcDeg", 6); // end
}
#endif

TEST_F(string_tests, test_ifind_use_elem_arrays)
{
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, " ", " ", 0);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "A", "A", 0);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "a", "A", 0);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "A", "a", 0);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "a", "a", 0);

    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "A", "B", 1); // end
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "a", "B", 1); // end
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "A", "b", 1); // end
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "a", "b", 1); // end

    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "", "", 0);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, " ", "", 0);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "ABCDEF", "ABCDEF", 0);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "abcdef", "ABCDEF", 0);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "ABCDEF", "abcdef", 0);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "abcdef", "abcdef", 0);

    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "", " ", 0); // end
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "ABCDEF", "ABCDEG", 6); // end
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "abcdef", "ABCDEG", 6); // end
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "ABCDEF", "abcdeg", 6); // end
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "abcdef", "abcdeg", 6); // end

    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "ABCDEF", "ABCDE", 0);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "abcdef", "ABCDE", 0);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "ABCDEF", "abcde", 0);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "abcdef", "abcde", 0);

    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "ABCDEF", "BCDEF", 1);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "abcdef", "BCDEF", 1);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "ABCDEF", "bcdef", 1);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "abcdef", "bcdef", 1);

    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "ABCDEF", "BCDE", 1);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "abcdef", "BCDE", 1);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "ABCDEF", "bcde", 1);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "abcdef", "bcde", 1);

    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "aBcDeF", "ABCDEF", 0);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "ABCDEF", "aBcDeF", 0);

    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "aBcDeF", "ABCDEG", 6); // end
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::ifind, "ABCDEF", "aBcDeg", 6); // end
}

TEST_F(string_tests, test_ifind_use_strings)
{
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, " ", " ", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "A", "A", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "a", "A", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "A", "a", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "a", "a", 0);

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "A", "B", 1); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "a", "B", 1); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "A", "b", 1); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "a", "b", 1); // end

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "", "", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, " ", "", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "ABCDEF", "ABCDEF", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "abcdef", "ABCDEF", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "ABCDEF", "abcdef", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "abcdef", "abcdef", 0);

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "", " ", 0); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "ABCDEF", "ABCDEFG", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "abcdef", "ABCDEFG", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "ABCDEF", "abcdefg", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "abcdef", "abcdefg", 6); // end

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "ABCDEF", "ABCDE", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "abcdef", "ABCDE", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "ABCDEF", "abcde", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "abcdef", "abcde", 0);

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "ABCDEF", "BCDEF", 1);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "abcdef", "BCDEF", 1);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "ABCDEF", "bcdef", 1);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "abcdef", "bcdef", 1);

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "ABCDEF", "BCDE", 1);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "abcdef", "BCDE", 1);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "ABCDEF", "bcde", 1);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "abcdef", "bcde", 1);

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "aBcDeF", "ABCDEF", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "ABCDEF", "aBcDeF", 0);

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "aBcDeF", "ABCDEG", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::ifind, "ABCDEF", "aBcDeg", 6); // end
}

#if PLUTO_UTILS_HAS_CXX_17
TEST_F(string_tests, test_irfind_use_elem_arrays)
{
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, " ", " ", 0);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "A", "A", 0);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "a", "A", 0);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "A", "a", 0);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "a", "a", 0);

    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "A", "B", 1); // end
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "a", "B", 1); // end
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "A", "b", 1); // end
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "a", "b", 1); // end

    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "", "", 0);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, " ", "", 1);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "ABCDEF", "ABCDEF", 0);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "abcdef", "ABCDEF", 0);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "ABCDEF", "abcdef", 0);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "abcdef", "abcdef", 0);

    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "", " ", 0); // end
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "ABCDEF", "ABCDEG", 6); // end
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "abcdef", "ABCDEG", 6); // end
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "ABCDEF", "abcdeg", 6); // end
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "abcdef", "abcdeg", 6); // end

    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "ABCDEF", "ABCDE", 0);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "abcdef", "ABCDE", 0);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "ABCDEF", "abcde", 0);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "abcdef", "abcde", 0);

    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "ABCDEF", "BCDEF", 1);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "abcdef", "BCDEF", 1);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "ABCDEF", "bcdef", 1);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "abcdef", "bcdef", 1);

    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "ABCDEF", "BCDE", 1);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "abcdef", "BCDE", 1);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "ABCDEF", "bcde", 1);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "abcdef", "bcde", 1);

    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "aBcDeF", "ABCDEF", 0);
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "ABCDEF", "aBcDeF", 0);

    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "aBcDeF", "ABCDEG", 6); // end
    TEST_ALL_ELEM_ARRAYS_4(ASSERT_EQ, pluto::irfind, "ABCDEF", "aBcDeg", 6); // end
}

TEST_F(string_tests, test_irfind_use_strings)
{
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, " ", " ", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "A", "A", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "a", "A", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "A", "a", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "a", "a", 0);

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "A", "B", 1); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "a", "B", 1); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "A", "b", 1); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "a", "b", 1); // end

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "", "", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, " ", "", 1);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "ABCDEF", "ABCDEF", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "abcdef", "ABCDEF", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "ABCDEF", "abcdef", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "abcdef", "abcdef", 0);

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "", " ", 0); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "ABCDEF", "ABCDEFG", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "abcdef", "ABCDEFG", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "ABCDEF", "abcdefg", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "abcdef", "abcdefg", 6); // end

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "ABCDEF", "ABCDE", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "abcdef", "ABCDE", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "ABCDEF", "abcde", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "abcdef", "abcde", 0);

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "ABCDEF", "BCDEF", 1);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "abcdef", "BCDEF", 1);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "ABCDEF", "bcdef", 1);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "abcdef", "bcdef", 1);

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "ABCDEF", "BCDE", 1);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "abcdef", "BCDE", 1);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "ABCDEF", "bcde", 1);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "abcdef", "bcde", 1);

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "aBcDeF", "ABCDEF", 0);
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "ABCDEF", "aBcDeF", 0);

    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "aBcDeF", "ABCDEG", 6); // end
    TEST_ALL_STRINGS_4(ASSERT_EQ, pluto::irfind, "ABCDEF", "aBcDeg", 6); // end
}
#endif

TEST_F(string_tests, test_contains_use_strings)
{
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::contains, " ", " ");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::contains, "A", "A");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::contains, "a", "a");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::contains, "a", "A");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::contains, "A", "a");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::contains, "A", "B");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::contains, "a", "B");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::contains, "A", "b");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::contains, "a", "b");

    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::contains, "", "");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::contains, " ", "");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::contains, "ABCDEF", "ABCDEF");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::contains, "abcdef", "abcdef");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::contains, "abcdef", "ABCDEF");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::contains, "ABCDEF", "abcdef");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::contains, "", " ");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::contains, "ABCDEF", "ABCDEG");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::contains, "abcdef", "ABCDEG");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::contains, "ABCDEF", "abcdeg");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::contains, "abcdef", "abcdeg");

    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::contains, "ABCDEF", "ABCDE");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::contains, "abcdef", "abcde");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::contains, "abcdef", "ABCDE");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::contains, "ABCDEF", "abcde");

    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::contains, "ABCDEF", "BCDEF");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::contains, "abcdef", "bcdef");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::contains, "abcdef", "BCDEF");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::contains, "ABCDEF", "bcdef");

    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::contains, "ABCDEF", "BCDE");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::contains, "abcdef", "bcde");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::contains, "abcdef", "BCDE");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::contains, "ABCDEF", "bcde");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::contains, "aBcDeF", "ABCDEF");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::contains, "ABCDEF", "aBcDeF");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::contains, "aBcDeF", "ABCDEG");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::contains, "ABCDEF", "aBcDeg");
}

TEST_F(string_tests, test_icontains_use_elem_arrays)
{
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::icontains, " ", " ");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::icontains, "A", "A");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::icontains, "a", "A");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::icontains, "A", "a");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::icontains, "a", "a");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::icontains, "A", "B");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::icontains, "a", "B");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::icontains, "A", "b");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::icontains, "a", "b");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::icontains, "", "");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::icontains, " ", "");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::icontains, "ABCDEF", "ABCDEF");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::icontains, "abcdef", "ABCDEF");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::icontains, "ABCDEF", "abcdef");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::icontains, "abcdef", "abcdef");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::icontains, "", " ");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::icontains, "ABCDEF", "ABCDEG");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::icontains, "abcdef", "ABCDEG");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::icontains, "ABCDEF", "abcdeg");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::icontains, "abcdef", "abcdeg");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::icontains, "ABCDEF", "ABCDE");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::icontains, "abcdef", "ABCDE");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::icontains, "ABCDEF", "abcde");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::icontains, "abcdef", "abcde");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::icontains, "ABCDEF", "BCDEF");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::icontains, "abcdef", "BCDEF");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::icontains, "ABCDEF", "bcdef");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::icontains, "abcdef", "bcdef");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::icontains, "ABCDEF", "BCDE");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::icontains, "abcdef", "BCDE");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::icontains, "ABCDEF", "bcde");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::icontains, "abcdef", "bcde");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::icontains, "aBcDeF", "ABCDEF");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_TRUE, pluto::icontains, "ABCDEF", "aBcDeF");

    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::icontains, "aBcDeF", "ABCDEG");
    TEST_ALL_ELEM_ARRAYS_3(ASSERT_FALSE, pluto::icontains, "ABCDEF", "aBcDeg");
}

TEST_F(string_tests, test_icontains_use_strings)
{
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::icontains, " ", " ");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::icontains, "A", "A");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::icontains, "a", "A");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::icontains, "A", "a");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::icontains, "a", "a");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::icontains, "A", "B");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::icontains, "a", "B");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::icontains, "A", "b");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::icontains, "a", "b");

    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::icontains, "", "");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::icontains, " ", "");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::icontains, "ABCDEF", "ABCDEF");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::icontains, "abcdef", "ABCDEF");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::icontains, "ABCDEF", "abcdef");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::icontains, "abcdef", "abcdef");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::icontains, "", " ");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::icontains, "ABCDEF", "ABCDEG");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::icontains, "abcdef", "ABCDEG");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::icontains, "ABCDEF", "abcdeg");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::icontains, "abcdef", "abcdeg");

    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::icontains, "ABCDEF", "ABCDE");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::icontains, "abcdef", "ABCDE");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::icontains, "ABCDEF", "abcde");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::icontains, "abcdef", "abcde");

    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::icontains, "ABCDEF", "BCDEF");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::icontains, "abcdef", "BCDEF");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::icontains, "ABCDEF", "bcdef");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::icontains, "abcdef", "bcdef");

    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::icontains, "ABCDEF", "BCDE");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::icontains, "abcdef", "BCDE");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::icontains, "ABCDEF", "bcde");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::icontains, "abcdef", "bcde");

    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::icontains, "aBcDeF", "ABCDEF");
    TEST_ALL_STRINGS_3(ASSERT_TRUE, pluto::icontains, "ABCDEF", "aBcDeF");

    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::icontains, "aBcDeF", "ABCDEG");
    TEST_ALL_STRINGS_3(ASSERT_FALSE, pluto::icontains, "ABCDEF", "aBcDeg");
}

TEST_F(string_tests, test_split_use_strings)
{
    TEST_ALL_STRINGS_5(ASSERT_EQ, pluto::split, "a", 1, "a");
    TEST_ALL_STRINGS_5(ASSERT_EQ, pluto::split, "A", 1, "A");

    TEST_ALL_STRINGS_5(ASSERT_EQ, pluto::split, "", 0,);
    TEST_ALL_STRINGS_5(ASSERT_EQ, pluto::split, "   ", 0,);
    TEST_ALL_STRINGS_5(ASSERT_EQ, pluto::split, "\t\n\v\f\r ", 0,);

    TEST_ALL_STRINGS_5(ASSERT_EQ, pluto::split, " a ", 1, "a");
    TEST_ALL_STRINGS_5(ASSERT_EQ, pluto::split, "a b c d e f", 6, "a", "b", "c", "d", "e", "f");
    TEST_ALL_STRINGS_5(ASSERT_EQ, pluto::split, "A B C D E F", 6, "A", "B", "C", "D", "E", "F");
    TEST_ALL_STRINGS_5(ASSERT_EQ, pluto::split, "a\tb\nc\vd\fe\rf", 6, "a", "b", "c", "d", "e", "f");
}

TEST_F(string_tests, test_split_separator_use_strings)
{
    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split, "a", "", 1, "a");
    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split, "a", " ", 1, "a");
    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split, "A", " ", 1, "A");

    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split, "", " ", 1, "");
    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split, "   ", " ", 4, "", "", "", "");
    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split, "\t\n\v\f\r ", " ", 2, "\t\n\v\f\r", "");

    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split, " a ", " ", 3, "", "a", "");
    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split, "abcdef", "", 6, "a", "b", "c", "d", "e", "f");
    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split, "a b c d e f", " ", 6, "a", "b", "c", "d", "e", "f");
    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split, "A B C D E F", " ", 6, "A", "B", "C", "D", "E", "F");
    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split, "a\tb\nc\vd\fe\rf", " ", 1, "a\tb\nc\vd\fe\rf");

    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split, "a:b:c:d:e:f", ":", 6, "a", "b", "c", "d", "e", "f");
    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split, "a;b;c;d;e;f", ";", 6, "a", "b", "c", "d", "e", "f");
    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split, "a-b-c-d-e-f", "-", 6, "a", "b", "c", "d", "e", "f");
    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split, "a#b#c#d#e#f", "#", 6, "a", "b", "c", "d", "e", "f");

    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split, "abcdefabcdef", "abc", 3, "", "def", "def");
    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split, "abcdefabcdef", "ace", 1, "abcdefabcdef");
}

TEST_F(string_tests, test_split_any_of_use_strings)
{
    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split_any_of, "a", " ", 1, "a");
    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split_any_of, "A", " ", 1, "A");

    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split_any_of, "", " ", 0);
    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split_any_of, "   ", " ", 0);
    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split_any_of, "\t\n\v\f\r ", " ", 1, "\t\n\v\f\r");

    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split_any_of, " a ", " ", 1, "a");
    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split_any_of, "a b c d e f", " ", 6, "a", "b", "c", "d", "e", "f");
    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split_any_of, "A B C D E F", " ", 6, "A", "B", "C", "D", "E", "F");
    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split_any_of, "a\tb\nc\vd\fe\rf", " ", 1, "a\tb\nc\vd\fe\rf");

    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split_any_of, "a:b:c:d:e:f", ":", 6, "a", "b", "c", "d", "e", "f");
    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split_any_of, "a;b;c;d;e;f", ";", 6, "a", "b", "c", "d", "e", "f");
    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split_any_of, "a-b-c-d-e-f", "-", 6, "a", "b", "c", "d", "e", "f");
    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split_any_of, "a#b#c#d#e#f", "#", 6, "a", "b", "c", "d", "e", "f");

    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split_any_of, "abcdefabcdef", "abc", 2, "def", "def");
    TEST_ALL_STRINGS_6(ASSERT_EQ, pluto::split_any_of, "abcdefabcdef", "ace", 6, "b", "d", "f", "b", "d", "f");
}

TEST_F(string_tests, test_join_iterators_use_strings)
{
    TEST_ALL_STRINGS_7(ASSERT_EQ, pluto::join, ",", "a", 1, "a");
    TEST_ALL_STRINGS_7(ASSERT_EQ, pluto::join, ",", "A", 1, "A");

    TEST_ALL_STRINGS_7(ASSERT_EQ, pluto::join, ",", "", 1, "");
    TEST_ALL_STRINGS_7(ASSERT_EQ, pluto::join, ",", ",,", 3, "", "", "");
    TEST_ALL_STRINGS_7(ASSERT_EQ, pluto::join, ",", ",,,,,", 6, "", "", "", "", "", "");

    TEST_ALL_STRINGS_7(ASSERT_EQ, pluto::join, "", "abcdef", 6, "a", "b", "c", "d", "e", "f");
    TEST_ALL_STRINGS_7(ASSERT_EQ, pluto::join, "", "ABCDEF", 6, "A", "B", "C", "D", "E", "F");

    TEST_ALL_STRINGS_7(ASSERT_EQ, pluto::join, " ", "a b c d e f", 6, "a", "b", "c", "d", "e", "f");
    TEST_ALL_STRINGS_7(ASSERT_EQ, pluto::join, " ", "A B C D E F", 6, "A", "B", "C", "D", "E", "F");

    TEST_ALL_STRINGS_7(ASSERT_EQ, pluto::join, ", ", "a, b, c, d, e, f", 6, "a", "b", "c", "d", "e", "f");
    TEST_ALL_STRINGS_7(ASSERT_EQ, pluto::join, ", ", "A, B, C, D, E, F", 6, "A", "B", "C", "D", "E", "F");
}

TEST_F(string_tests, test_join_container_use_strings)
{
    TEST_ALL_STRINGS_8(ASSERT_EQ, pluto::join, ",", "a", 1, "a");
    TEST_ALL_STRINGS_8(ASSERT_EQ, pluto::join, ",", "A", 1, "A");

    TEST_ALL_STRINGS_8(ASSERT_EQ, pluto::join, ",", "", 1, "");
    TEST_ALL_STRINGS_8(ASSERT_EQ, pluto::join, ",", ",,", 3, "", "", "");
    TEST_ALL_STRINGS_8(ASSERT_EQ, pluto::join, ",", ",,,,,", 6, "", "", "", "", "", "");

    TEST_ALL_STRINGS_8(ASSERT_EQ, pluto::join, "", "abcdef", 6, "a", "b", "c", "d", "e", "f");
    TEST_ALL_STRINGS_8(ASSERT_EQ, pluto::join, "", "ABCDEF", 6, "A", "B", "C", "D", "E", "F");

    TEST_ALL_STRINGS_8(ASSERT_EQ, pluto::join, " ", "a b c d e f", 6, "a", "b", "c", "d", "e", "f");
    TEST_ALL_STRINGS_8(ASSERT_EQ, pluto::join, " ", "A B C D E F", 6, "A", "B", "C", "D", "E", "F");

    TEST_ALL_STRINGS_8(ASSERT_EQ, pluto::join, ", ", "a, b, c, d, e, f", 6, "a", "b", "c", "d", "e", "f");
    TEST_ALL_STRINGS_8(ASSERT_EQ, pluto::join, ", ", "A, B, C, D, E, F", 6, "A", "B", "C", "D", "E", "F");
}

TEST_F(string_tests, test_replace_use_strings)
{
    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace, " ", "", "", " ");
    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace, "a", " ", "b", "a");
    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace, "A", " ", "b", "A");
    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace, "aaa", "", "b", "bababab");

    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace, "", " ", "a", "");
    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace, "   ", " ", "a", "aaa");
    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace, "\t\n\v\f\r ", " ", "a", "\t\n\v\f\ra");

    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace, " a ", " ", "b", "bab");
    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace, "a b c d e f", " ", "_", "a_b_c_d_e_f");
    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace, "A B C D E F", " ", "_", "A_B_C_D_E_F");
    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace, "a\tb\nc\vd\fe\rf", " ", "_", "a\tb\nc\vd\fe\rf");

    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace, "a:b:c:d:e:f", ":", "_", "a_b_c_d_e_f");
    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace, "a;b;c;d;e;f", ";", "_", "a_b_c_d_e_f");
    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace, "a-b-c-d-e-f", "-", "_", "a_b_c_d_e_f");
    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace, "a#b#c#d#e#f", "#", "_", "a_b_c_d_e_f");

    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace, "abcdefabcdef", "abc", "def", "defdefdefdef");
    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace, "abcdefabcdef", "ace", "def", "abcdefabcdef");
}

TEST_F(string_tests, test_replace_any_of_use_strings)
{
    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace_any_of, " ", "", "", " ");
    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace_any_of, "a", " ", "b", "a");
    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace_any_of, "A", " ", "b", "A");
    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace_any_of, "aaa", "", "b", "aaa");

    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace_any_of, "", " ", "a", "");
    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace_any_of, "   ", " ", "a", "aaa");
    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace_any_of, "\t\n\v\f\r ", " ", "a", "\t\n\v\f\ra");

    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace_any_of, " a ", " ", "b", "bab");
    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace_any_of, "a b c d e f", " ", "_", "a_b_c_d_e_f");
    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace_any_of, "A B C D E F", " ", "_", "A_B_C_D_E_F");
    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace_any_of, "a b c d e f", "abcdef", "x", "x x x x x x");
    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace_any_of, "A B C D E F", "ABCDEF", "X", "X X X X X X");
    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace_any_of, "a\tb\nc\vd\fe\rf", " ", "_", "a\tb\nc\vd\fe\rf");

    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace_any_of, "a:b:c:d:e:f", ":;-#", "_", "a_b_c_d_e_f");
    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace_any_of, "a;b;c;d;e;f", ":;-#", "_", "a_b_c_d_e_f");
    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace_any_of, "a-b-c-d-e-f", ":;-#", "_", "a_b_c_d_e_f");
    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace_any_of, "a#b#c#d#e#f", ":;-#", "_", "a_b_c_d_e_f");

    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace_any_of, "abcdefabcdef", "abc", "a", "aaadefaaadef");
    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace_any_of, "abcdefabcdef", "abc", "ab", "abababdefabababdef");
    TEST_ALL_STRINGS_9(ASSERT_EQ, pluto::replace_any_of, "abcdefabcdef", "ace", "ab", "abbabdabfabbabdabf");
}

TEST_F(string_tests, test_lstrip_use_strings)
{
    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::lstrip, "a", "a");
    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::lstrip, "A", "A");
    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::lstrip, "\t\n\v\f\r", "");

    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::lstrip, " a ", "a ");
    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::lstrip, " A ", "A ");
    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::lstrip, "\t\n\v\f\ra\t\n\v\f\r", "a\t\n\v\f\r");

    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::lstrip, " a b c ", "a b c ");
    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::lstrip, "\t\n\v\f\ra b c\t\n\v\f\r", "a b c\t\n\v\f\r");
}

TEST_F(string_tests, test_rstrip_use_strings)
{
    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::rstrip, "a", "a");
    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::rstrip, "A", "A");
    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::rstrip, "\t\n\v\f\r", "");

    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::rstrip, " a ", " a");
    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::rstrip, " A ", " A");
    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::rstrip, "\t\n\v\f\ra\t\n\v\f\r", "\t\n\v\f\ra");

    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::rstrip, " a b c ", " a b c");
    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::rstrip, "\t\n\v\f\ra b c\t\n\v\f\r", "\t\n\v\f\ra b c");
}

TEST_F(string_tests, test_strip_use_strings)
{
    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::strip, "a", "a");
    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::strip, "A", "A");
    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::strip, "\t\n\v\f\r", "");

    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::strip, " a ", "a");
    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::strip, " A ", "A");
    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::strip, "\t\n\v\f\ra\t\n\v\f\r", "a");

    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::strip, " a b c ", "a b c");
    TEST_ALL_STRINGS_2(ASSERT_EQ, pluto::strip, "\t\n\v\f\ra b c\t\n\v\f\r", "a b c");
}

TEST_F(string_tests, test_bin_use_string)
{
    ASSERT_EQ(pluto::bin(0), "0b0");
    ASSERT_EQ(pluto::bin(1), "0b1");
    ASSERT_EQ(pluto::bin(2), "0b10");
    ASSERT_EQ(pluto::bin(255), "0b11111111");
    ASSERT_EQ(pluto::bin(256), "0b100000000");
    ASSERT_EQ(pluto::bin(-1), "0b11111111111111111111111111111111");
    ASSERT_EQ(pluto::bin(-2), "0b11111111111111111111111111111110");

    ASSERT_EQ(pluto::bin(0u), "0b0");
    ASSERT_EQ(pluto::bin(1u), "0b1");
    ASSERT_EQ(pluto::bin(2u), "0b10");
    ASSERT_EQ(pluto::bin(255u), "0b11111111");
    ASSERT_EQ(pluto::bin(256u), "0b100000000");
    ASSERT_EQ(pluto::bin(-1u), "0b11111111111111111111111111111111");
    ASSERT_EQ(pluto::bin(-2u), "0b11111111111111111111111111111110");

    ASSERT_EQ(pluto::bin(0ll), "0b0");
    ASSERT_EQ(pluto::bin(1ll), "0b1");
    ASSERT_EQ(pluto::bin(2ll), "0b10");
    ASSERT_EQ(pluto::bin(255ll), "0b11111111");
    ASSERT_EQ(pluto::bin(256ll), "0b100000000");
    ASSERT_EQ(pluto::bin(-1ll), "0b1111111111111111111111111111111111111111111111111111111111111111");
    ASSERT_EQ(pluto::bin(-2ll), "0b1111111111111111111111111111111111111111111111111111111111111110");

    ASSERT_EQ(pluto::bin(0ull), "0b0");
    ASSERT_EQ(pluto::bin(1ull), "0b1");
    ASSERT_EQ(pluto::bin(2ull), "0b10");
    ASSERT_EQ(pluto::bin(255ull), "0b11111111");
    ASSERT_EQ(pluto::bin(256ull), "0b100000000");
    ASSERT_EQ(pluto::bin(-1ull), "0b1111111111111111111111111111111111111111111111111111111111111111");
    ASSERT_EQ(pluto::bin(-2ull), "0b1111111111111111111111111111111111111111111111111111111111111110");
}

TEST_F(string_tests, test_oct_use_string)
{
    ASSERT_EQ(pluto::oct(0), "0o0");
    ASSERT_EQ(pluto::oct(1), "0o1");
    ASSERT_EQ(pluto::oct(8), "0o10");
    ASSERT_EQ(pluto::oct(255), "0o377");
    ASSERT_EQ(pluto::oct(256), "0o400");
    ASSERT_EQ(pluto::oct(-1), "0o37777777777");
    ASSERT_EQ(pluto::oct(-9), "0o37777777767");

    ASSERT_EQ(pluto::oct(0u), "0o0");
    ASSERT_EQ(pluto::oct(1u), "0o1");
    ASSERT_EQ(pluto::oct(8u), "0o10");
    ASSERT_EQ(pluto::oct(255u), "0o377");
    ASSERT_EQ(pluto::oct(256u), "0o400");
    ASSERT_EQ(pluto::oct(-1u), "0o37777777777");
    ASSERT_EQ(pluto::oct(-9u), "0o37777777767");

    ASSERT_EQ(pluto::oct(0ll), "0o0");
    ASSERT_EQ(pluto::oct(1ll), "0o1");
    ASSERT_EQ(pluto::oct(8ll), "0o10");
    ASSERT_EQ(pluto::oct(255ll), "0o377");
    ASSERT_EQ(pluto::oct(256ll), "0o400");
    ASSERT_EQ(pluto::oct(-1ll), "0o1777777777777777777777");
    ASSERT_EQ(pluto::oct(-9ll), "0o1777777777777777777767");

    ASSERT_EQ(pluto::oct(0ull), "0o0");
    ASSERT_EQ(pluto::oct(1ull), "0o1");
    ASSERT_EQ(pluto::oct(8ull), "0o10");
    ASSERT_EQ(pluto::oct(255ull), "0o377");
    ASSERT_EQ(pluto::oct(256ull), "0o400");
    ASSERT_EQ(pluto::oct(-1ull), "0o1777777777777777777777");
    ASSERT_EQ(pluto::oct(-9ull), "0o1777777777777777777767");
}

TEST_F(string_tests, test_hex_use_string)
{
    ASSERT_EQ(pluto::hex(0), "0x0");
    ASSERT_EQ(pluto::hex(1), "0x1");
    ASSERT_EQ(pluto::hex(16), "0x10");
    ASSERT_EQ(pluto::hex(255), "0xff");
    ASSERT_EQ(pluto::hex(256), "0x100");
    ASSERT_EQ(pluto::hex(-1), "0xffffffff");
    ASSERT_EQ(pluto::hex(-17), "0xffffffef");

    ASSERT_EQ(pluto::hex(0u), "0x0");
    ASSERT_EQ(pluto::hex(1u), "0x1");
    ASSERT_EQ(pluto::hex(16u), "0x10");
    ASSERT_EQ(pluto::hex(255u), "0xff");
    ASSERT_EQ(pluto::hex(256u), "0x100");
    ASSERT_EQ(pluto::hex(-1u), "0xffffffff");
    ASSERT_EQ(pluto::hex(-17u), "0xffffffef");

    ASSERT_EQ(pluto::hex(0ll), "0x0");
    ASSERT_EQ(pluto::hex(1ll), "0x1");
    ASSERT_EQ(pluto::hex(16ll), "0x10");
    ASSERT_EQ(pluto::hex(255ll), "0xff");
    ASSERT_EQ(pluto::hex(256ll), "0x100");
    ASSERT_EQ(pluto::hex(-1ll), "0xffffffffffffffff");
    ASSERT_EQ(pluto::hex(-17ll), "0xffffffffffffffef");

    ASSERT_EQ(pluto::hex(0ull), "0x0");
    ASSERT_EQ(pluto::hex(1ull), "0x1");
    ASSERT_EQ(pluto::hex(16ull), "0x10");
    ASSERT_EQ(pluto::hex(255ull), "0xff");
    ASSERT_EQ(pluto::hex(256ull), "0x100");
    ASSERT_EQ(pluto::hex(-1ull), "0xffffffffffffffff");
    ASSERT_EQ(pluto::hex(-17ull), "0xffffffffffffffef");
}
