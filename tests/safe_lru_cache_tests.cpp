...
TEST_F(safe_lru_cache_tests, test_cache_sanity)
{
    ...
    ASSERT_TRUE(safeCache.insert(1, 1));
...
TEST_F(safe_lru_cache_tests, test_change_capacity)
{
    ...
    {
        ASSERT_TRUE(safeCache.insert(i, i));
    }
...
TEST_F(safe_lru_cache_tests, test_insert_evicts_oldest)
{
    ...
    ASSERT_TRUE(safeCache.insert(SAFE_CACHE_CAPACITY + 1, SAFE_CACHE_CAPACITY + 1));

    std::size_t value{ 0 };
    ASSERT_FALSE(safeCache.get(1, value));
}

TEST_F(safe_lru_cache_tests, test_insert_does_not_assign)
{
    for (std::size_t i{ 1 }; i <= SAFE_CACHE_CAPACITY; ++i)
    {
        ASSERT_TRUE(safeCache.insert(i, i));
    ...
    ASSERT_FALSE(safeCache.insert(1, 2));
...
TEST_F(safe_lru_cache_tests, test_insert_does_not_move_to_front)
{
    for (std::size_t i{ 1 }; i <= SAFE_CACHE_CAPACITY; ++i)
    {
        ASSERT_TRUE(safeCache.insert(i, i));
    ...
    ASSERT_FALSE(safeCache.insert(1, 1));
    ASSERT_TRUE(safeCache.insert(SAFE_CACHE_CAPACITY + 1, SAFE_CACHE_CAPACITY + 1));
...
TEST_F(safe_lru_cache_tests, test_insert_or_assign_evicts_oldest)
{
    for (std::size_t i{ 1 }; i <= SAFE_CACHE_CAPACITY; ++i)
    {
        ASSERT_TRUE(safeCache.insert(i, i));
    ...
    ASSERT_TRUE(safeCache.insert_or_assign(SAFE_CACHE_CAPACITY + 1, SAFE_CACHE_CAPACITY + 1));

    std::size_t value{ 0 };
    ASSERT_FALSE(safeCache.get(1, value));
}

TEST_F(safe_lru_cache_tests, test_insert_or_assign_does_assign)
{
    for (std::size_t i{ 1 }; i <= SAFE_CACHE_CAPACITY; ++i)
    {
        ASSERT_TRUE(safeCache.insert(i, i));
    ...
    ASSERT_FALSE(safeCache.insert_or_assign(1, 2));
...
TEST_F(safe_lru_cache_tests, test_insert_or_assign_moves_to_front)
{
    for (std::size_t i{ 1 }; i <= SAFE_CACHE_CAPACITY; ++i)
    {
        ASSERT_TRUE(safeCache.insert(i, i));
    ...
    ASSERT_FALSE(safeCache.insert_or_assign(1, 1));
    ASSERT_TRUE(safeCache.insert(SAFE_CACHE_CAPACITY + 1, SAFE_CACHE_CAPACITY + 1));
...
TEST_F(safe_lru_cache_tests, test_get_moves_to_front)
{
    ...
    std::size_t unused;
    ASSERT_TRUE(safeCache.get(1, unused));
    ASSERT_TRUE(safeCache.insert(SAFE_CACHE_CAPACITY + 1, SAFE_CACHE_CAPACITY + 1));
...
TEST_F(safe_lru_cache_tests, test_remove)
{
    ...
    ASSERT_TRUE(safeCache.remove(1));
